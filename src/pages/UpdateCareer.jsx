    ...
    const handleSave = async () => {
        ...
        try {
            await careersAPI.update(careerId, {
                ...
                youtubePlaylists: youtube.filter(y => y.title),
                v: career?.v ?? 1,
            })
            if (picture) {
        ...
        <AdminLayout>
            ...
            <div className="adm-form-card" style={{ marginBottom:14 }}>
                <div style={sectionHeader}>
                    <span style={{ fontWeight:700, fontSize:14, color:'#1A1A1A' }}>Courses</span>
                    <button style={addBtnStyle}
                        onClick={() => navigate(`/add-course?careerId=${careerId}&career=${encodeURIComponent(title)}`)}>
                        <i className="bi bi-plus-lg"></i> Add new course
                    </button>
                </div>

                <div style={{ marginBottom:8, fontSize:12, fontWeight:600, color:'#999' }}>Courses</div>
                {courses.length === 0 && (
                    <div style={{ color:'#bbb', fontSize:13, textAlign:'center', padding:'10px 0' }}>No courses yet.</div>
                )}
                {courses.map(c => (
                    <div key={c.id} style={rowStyle}>
                        <span style={{ fontSize:13, color:'#333' }}>{c.title}</span>
                        <button style={delBtnStyle} onClick={() => handleDeleteCourse(c.id)} title="Delete">
            ...
            <div className="adm-form-card" style={{ marginBottom:14 }}>
                <div style={sectionHeader}>
                    <span style={{ fontWeight:700, fontSize:14, color:'#1A1A1A' }}>Books</span>
                    <button style={addBtnStyle}
                        onClick={() => navigate(`/add-book?careerId=${careerId}&career=${encodeURIComponent(title)}`)}>
                        <i className="bi bi-plus-lg"></i> Add new book
                    </button>
                </div>

                <div style={{ marginBottom:8, fontSize:12, fontWeight:600, color:'#999' }}>Books</div>
                {books.length === 0 && (
                    <div style={{ color:'#bbb', fontSize:13, textAlign:'center', padding:'10px 0' }}>No books yet.</div>
                )}
                {books.map(b => (
                    <div key={b.id} style={rowStyle}>
                        <span style={{ fontSize:13, color:'#333' }}>{b.title}</span>
                        <button style={delBtnStyle} onClick={() => handleDeleteBook(b.id)} title="Delete">
                            <i className="bi bi-trash3"></i>
                        </button>
                    </div>
                ))}
            ...
            <div className="adm-form-card" style={{ marginBottom:14 }}>
                <div style={sectionHeader}>
                    <span style={{ fontWeight:700, fontSize:14, color:'#1A1A1A' }}>YouTube Resources</span>
                    <button style={addBtnStyle}
                        onClick={() => navigate(`/add-resource?careerId=${careerId}&career=${encodeURIComponent(title)}`)}>
                        <i className="bi bi-plus-lg"></i> Add new resource
                    </button>
                </div>

                <div style={{ marginBottom:8, fontSize:12, fontWeight:600, color:'#999' }}>YouTube Resources</div>
                {youtube.length === 0 && (
                    <div style={{ color:'#bbb', fontSize:13, textAlign:'center', padding:'10px 0' }}>No YouTube resources yet.</div>
                )}
                {youtube.map(y => (
                    <div key={y.id} style={rowStyle}>
                        <span style={{ fontSize:13, color:'#333' }}>{y.title}</span>
                        <button style={delBtnStyle} onClick={() => handleDeleteYoutube(y.id)} title="Delete">
                            <i className="bi bi-trash3"></i>
                        </button>
                    </div>
                ))}
